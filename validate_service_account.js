...
app.post('/send-otp', async (req, res) => {
  ...
  try {
    ...
    if (waFrom) {
      const m = await twilioClient.messages.create({ from: waFrom, to: `whatsapp:${phone}`, body: `رمز التحقق: ${otp}` });
      console.log('WA sid', m.sid);
      channel = 'whatsapp';
    }
  } catch (e) {
    bad('WA send failed: ' + (e && e.message));
  }
...
app.post('/verify-otp', async (req, res) => {
  ...
  try {
    ...
    const token = await admin.auth().createCustomToken(uid, { provider: 'whatsapp' });
    otps.delete(phone);
